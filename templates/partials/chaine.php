 ...
 *
 * `$variante` vaut `pied` (une ligne discrète sous le logotype) ou `bande`
 * (le bandeau au filet de la page des actualités).
 *
...
<?php else: ?>
  <div class="foot__suivre">
    <p class="foot__suivre-t"><?= t('chaine.suivre') ?></p>
    <a class="chaine" href="<?= View::e($urlChaine) ?>"<?= $sortant ?>
       aria-label="<?= t('chaine.aria') ?>">
      <?= $glyphe ?>
      <span><?= t('chaine.lien') ?>
        <?php /* La flèche oblique signale le lien sortant — même marque que
                 « lire l'article original » sur une fiche d'actualité. */ ?>
        <span class="chaine__fleche" aria-hidden="true">&#8599;</span>
      </span>
    </a>
    <p class="foot__suivre-n"><?= t('chaine.pied_note') ?></p>
  </div>
<?php endif; ?>
